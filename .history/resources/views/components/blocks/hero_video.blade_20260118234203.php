...
@php
    ...
    // FIX: Strict check. If key exists, use it. Only default to true if key is COMPLETELY missing.
    $showButton = isset($data['show_button']) ? filter_var($data['show_button'], FILTER_VALIDATE_BOOLEAN) : true;
        ...
        if (this.inEditor) {
            window.addEventListener('message', (e) => {
                if (e.data.type === 'UPDATE_BLOCK_FIELD' && e.data.index == this.$el.dataset.blockIndex) {
                     console.log('[DEBUG] Editor Update Received:', e.data.field, e.data.value);
                     ...
                     if (e.data.field === 'show_button') this.showButton = e.data.value;
                     if (e.data.field === 'template') this.template = e.data.value;
                     
                     // Live Preview for Backgrounds & Spots
                     if (e.data.field === 'backgrounds') {
                         console.log('[DEBUG] Processing Backgrounds Update. Raw Data:', e.data.value);
                         ...
                         this.slides = e.data.value.map((bg, idx) => {
                             console.log(`[DEBUG] Mapping Slide ${idx}, Spots:`, bg.spots);
                             return {
                         ...
                         console.log('[DEBUG] Updated this.slides:', this.slides);
                     }
                }
            });
        }
