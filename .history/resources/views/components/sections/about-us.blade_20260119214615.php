...
@php
    ...
    // Images - Logic moved to home.blade.php to avoid double wrapping
    $mainImage = !empty($data['main_image']) 
        ? (filter_var($data['main_image'], FILTER_VALIDATE_URL) ? $data['main_image'] : \Illuminate\Support\Facades\Storage::url($data['main_image'])) 
        : 'https://placehold.co/600x800?text=Adventure';

    $secondaryImage = !empty($data['secondary_image']) 
        ? (filter_var($data['secondary_image'], FILTER_VALIDATE_URL) ? $data['secondary_image'] : \Illuminate\Support\Facades\Storage::url($data['secondary_image'])) 
        : 'https://placehold.co/500x500?text=Joy';
