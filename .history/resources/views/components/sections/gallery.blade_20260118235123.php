...
@php
    ...
    $title = $data['title'] ?? 'Capture The <span class="text-brand-primary font-hand italic">Moments</span>';
...
<section class="py-16 md:py-24 bg-white relative font-sans overflow-hidden">
    ...
    <div class="container mx-auto px-6 md:px-12 lg:px-20 relative z-10">
        ...
        <div class="text-center max-w-3xl mx-auto mb-12">
            <span class="inline-block bg-orange-100 text-orange-500 font-hand text-xl px-4 py-1 rounded-full mb-3 transform -rotate-1">
                {{ $badge }}
            </span>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[200px]">
            ...
            @foreach($images as $img)
                @php 
                    $imgUrl = !empty($img['image']) && filter_var($img['image'], FILTER_VALIDATE_URL) ? $img['image'] : \Illuminate\Support\Facades\Storage::url($img['image']);
                    // Quick fix for placeholder URLs which are valid URLs
                    if (str_contains($img['image'], 'placehold.co')) { $imgUrl = $img['image']; }
                    $sizeClass = getGridClasses($img['size'] ?? 'small');
                @endphp
