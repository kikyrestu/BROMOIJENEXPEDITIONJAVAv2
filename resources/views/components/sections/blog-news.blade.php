...
@php
    $badge = $data['badge'] ?? 'Blog & News';
    
    // Defaults
    $defaultPrefix = 'Explore Blogs';
    $defaultSuffix = 'And News';
    ...
    $showFooterBtn = $data['show_footer_btn'] ?? false;
    $btnText = $data['button_text'] ?? 'View All Articles';
    ...
    foreach ($rawPosts as $post) {
        if ($post instanceof \App\Models\Blog) {
             $blogMedium = (!empty($post->thumbnail_path)) ? \App\Services\ImageOptimizationService::getMediumUrl($post->thumbnail_path) : null;
             $normalizedPosts[] = [
                'title' => $post->title,
                'image' => (!empty($post->thumbnail_path)) ? \Illuminate\Support\Facades\Storage::url($post->thumbnail_path) : 'https://placehold.co/800x600?text=' . urlencode($post->title),
                'image_medium' => $blogMedium,
    ...
    <div class="container mx-auto px-6 md:px-12 lg:px-20 relative z-10">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
            <div class="text-left">
                ...
                @if($description)
                    <p data-live="description" class="mt-4 text-slate-600 font-medium text-lg leading-relaxed max-w-2xl">
                        {!! nl2br(e($description)) !!}
                    </p>
                @endif
            </div>
            ...
            @foreach($normalizedPosts as $post)
                <div class="group h-full flex flex-col bg-white rounded-3xl overflow-hidden shadow-[0_10px_40px_-5px_rgba(0,0,0,0.05)] border border-slate-100 hover:shadow-[0_20px_50px_-5px_rgba(0,0,0,0.1)] transition-all duration-300 hover:-translate-y-1">
                    ...
                    <div class="relative w-full aspect-[16/10] overflow-hidden">
                        <img src="{{ $post['image_medium'] ?? $post['image'] }}" alt="{{ $post['title'] }}" loading="lazy"
                             @if(!empty($post['image_medium'])) srcset="{{ $post['image_medium'] }} 600w, {{ $post['image'] }} 1080w" sizes="(max-width: 768px) 100vw, 33vw" @endif
        ...
        <div data-live-visible="show_footer_btn" class="mt-12 flex justify-center" style="{{ !$showFooterBtn ? 'display: none;' : '' }}">
            <a href="{{ $btnUrl }}" class="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-brand-primary text-white font-bold hover:bg-brand-dark transition-all shadow-lg hover:shadow-xl hover:-translate-y-1">
                {{ $btnText }}
