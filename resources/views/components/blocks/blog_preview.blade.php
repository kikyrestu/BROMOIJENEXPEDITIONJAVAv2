...
@php
    // Mock data for posts since we don't have a blog model connected yet
    $posts = [
        [
...
<section class="py-16 md:py-20 bg-white relative font-sans overflow-hidden" data-block-index="{{ $attributes->get('data-block-index') }}">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
            <div class="text-left">
                <span class="inline-block bg-orange-100 text-orange-500 font-hand text-xl px-4 py-1 rounded-full mb-2 transform -rotate-1" data-field="badge_text">
                    {!! $data['badge_text'] ?? 'Blog & News' !!}
                </span>
                <h2 class="text-4xl md:text-5xl font-extrabold text-brand-dark tracking-tight leading-tight">
                     <span data-field="title_prefix">{!! $data['title_prefix'] ?? 'Explore Blogs' !!}</span> 
                     <span class="text-brand-primary font-hand italic" data-field="title_suffix">{!! $data['title_suffix'] ?? 'And News' !!}</span>
                </h2>
            </div>
            
            <a href="#" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-full border border-slate-200 text-brand-dark font-bold hover:bg-brand-primary hover:text-white hover:border-brand-primary transition-all group self-start md:self-end text-sm">
                <span data-field="button_text">{!! $data['button_text'] ?? 'See More Articles' !!}</span>
            ...
            @foreach(array_slice($posts, 0, 3) as $post)
                <div class="group h-full flex flex-col bg-white rounded-3xl overflow-hidden shadow-[0_10px_40px_-5px_rgba(0,0,0,0.05)] border border-slate-100 hover:shadow-[0_20px_50px_-5px_rgba(0,0,0,0.1)] transition-all duration-300 hover:-translate-y-1">
                    ...
                    <div class="relative w-full aspect-[16/10] overflow-hidden">
                        <img src="{{ $post['image'] }}" alt="{{ $post['title'] }}" loading="lazy" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                            <a href="#" class="inline-flex items-center gap-2 text-brand-dark font-bold text-sm group/link hover:text-brand-primary transition-colors">
                                Read Article
