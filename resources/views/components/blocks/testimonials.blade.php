...
@php
    ...
    if ($dbTestimonials->isNotEmpty()) {
        ...
        $testimonials = $data['testimonials_list'] ?? [
            [
                'name' => 'Sarah Jenner',
...
<section class="py-16 md:py-24 bg-[#f9f9f9] relative font-sans overflow-hidden" data-block-index="{{ $attributes->get('data-block-index') }}">
    ...
    <div class="container mx-auto px-6 mb-12 relative z-10 text-center">
        <span class="inline-block bg-orange-100 text-orange-500 font-hand text-2xl md:text-3xl px-4 py-2 rounded-full mb-3 transform -rotate-2" data-field="badge_text">
            {!! $data['badge_text'] ?? 'Community Love' !!}
        </span>
        <h2 class="text-4xl md:text-6xl font-extrabold text-brand-dark tracking-tight leading-tight">
            <span data-field="title_prefix">{!! $data['title_prefix'] ?? 'Trusted by' !!}</span> 
            <span class="text-brand-primary" data-field="title_suffix">{!! $data['title_suffix'] ?? 'Adventurers' !!}</span>
        </h2>
    </div>
                ...
                @foreach ($testimonials as $testimonial)
                    <div class="w-[280px] md:w-[400px] bg-white border border-slate-100 p-6 md:p-8 rounded-[30px] shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-xl transition-all duration-300 flex-shrink-0 flex flex-col relative group/card">
                        ...
                        <div class="flex items-center gap-4 mt-auto relative z-10 border-t border-slate-50 pt-6">
                            <img src="{{ $testimonial['avatar'] }}" alt="{{ $testimonial['name'] }}" loading="lazy" class="w-12 h-12 rounded-full border border-slate-100">
                            <div>
