...
<section class="py-16 md:py-24 bg-white relative font-sans overflow-hidden" data-block-index="{{ $attributes->get('data-block-index') }}" x-data="{ showLightbox: false, activeSrc: '', activeAlt: '' }">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[200px]">
                @foreach($galleryItems as $index => $item)
                    ...
                    <div class="{{ $sizeClass }} group relative rounded-3xl overflow-hidden cursor-pointer"
                        ...
                        <img src="{{ $item->display_url }}" alt="{{ $item->alt_text ?? $item->title }}" loading="lazy" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
    ...
    {{-- Lightbox --}}
    <div x-show="showLightbox" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @keydown.escape.window="showLightbox = false" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95" style="display: none;" x-cloak>
