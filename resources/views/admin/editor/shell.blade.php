                            ...
                            <div x-show="activeBlock === index" x-collapse class="p-5 space-y-4 bg-white">
                                ...
                                <template x-if="block.type === 'gallery'">
                                    <div class="space-y-4">
                                        ...
                                        <!-- Gallery Images - Managed via Filament CMS -->
                                        <div class="p-3 bg-amber-50 rounded-lg border border-amber-200">
                                            <div class="flex items-start gap-2">
                                                ...
                                                <div>
                                                    <p class="text-xs font-bold text-amber-700">Kelola Gambar via Admin Panel</p>
                                                    <p class="text-xs text-amber-600 mt-1">Untuk menambah/mengubah gambar Gallery, silakan edit halaman ini melalui <strong>Resources → Pages → Edit</strong> di Admin Panel Filament.</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <!-- DESTINATIONS GRID - Auto from DB -->
                                    ...
                                    <div class="space-y-4">
                                        <div class="p-3 bg-green-50 rounded-lg border border-green-200">
                                            <div class="flex items-start gap-2">
                                                ...
                                                <div>
                                                    <p class="text-xs font-bold text-green-700">Auto-Load dari Database</p>
                                                    <p class="text-xs text-green-600 mt-1">Section ini otomatis menampilkan <strong>semua Destinations</strong> dari menu <strong>Resources → Destinations</strong> di Admin Panel.</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            return {
                ...
                prepareData() {
                    this.page.content.forEach(block => {
                        // AUTO-MIGRATE LEGACY DATA (Fix for empty forms)
                        if(['exclusive_destinations', 'package_slider', 'testimonials_marquee', 'blog_news', 'gallery'].includes(block.type)) {
                             // Migrate Badge
                             if (!block.data.badge_text && block.data.badge) {
                                 block.data.badge_text = block.data.badge;
                             }
                             // Migrate Title
                             if ((!block.data.title_prefix && !block.data.title_suffix) && block.data.title) {
                                 let t = block.data.title;
                                 let prefix = t;
                                 let suffix = '';
                                 
                                 // Attempt to parse standard pattern: "Prefix <span...>Suffix</span>"
                                 // First check if it contains a span
                                 if (t.includes('<span')) {
                                     // Regex to capture text before span (Prefix) and text inside span (Suffix)
                                     const match = t.match(/^(.*?)<span[^>]*>(.*?)<\/span>$/);
                                     if (match) {
                                         prefix = match[1].trim(); // Group 1: Prefix
                                         suffix = match[2].trim(); // Group 2: Suffix
                                     } else {
                                         // Fallback: Strip HTML
                                         prefix = t.replace(/<[^>]*>?/gm, '').trim();
                                     }
                                 }
                                 
                                 block.data.title_prefix = prefix;
                                 block.data.title_suffix = suffix;
                             }
                        }
                ...
                handleMediaSelection(asset) {
                    ...
                    console.log(`[Media Debug] Selected Asset for Block[${index}].${field}:`, asset);
                    ...
                    if (field === 'backgrounds') {
                        ...
                        this.page.content[index].data.backgrounds.push({
                            ...
                            id: asset.id,
                            url: asset.url,
                            mime_type: asset.mime_type || asset.type
                        });
